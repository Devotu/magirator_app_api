...
MATCH (a:Player) WHERE a.id = 11 CREATE (a)-[:Possess]->(n:Deck:Active:PERSISTENT { id:22 })-[:Currently]->(d:Data {created:TIMESTAMP(), name:"Deck 3", format:"BOOSTER_HANDOVER", theme:"Testing", black:false, white:false, red:false, green:false, blue:false, colorless:false});
MATCH (p:Player) WHERE p.id = 11 CREATE (t:Token { created:TIMESTAMP(), token:"1234567890ABCDEF" })-[:Grants]->(p);
MATCH (d:Deck) WHERE d.id = 20 CREATE (d)-[:Got]->(r:Result:FINAL { id:30, created:TIMESTAMP(), place:1, comment:"Första resultatet i appen" });